...
const walk = (jsPath, modules, options) => {
    ...
    const extract = ({ node }) => {
        let importPath = (lodash_1.get(node, 'callee.name') === 'require' ? lodash_1.get(node, 'arguments[0].value') : '') || lodash_1.get(node, 'source.value');
        if (!importPath) {
            return;
        }
        try {
        ...
        const absoluteId = nativeComponent_1.getPath(jsPath, importPath);
