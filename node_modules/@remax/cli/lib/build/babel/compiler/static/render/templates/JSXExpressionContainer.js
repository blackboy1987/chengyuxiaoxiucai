...
function JSXExpressionContainer(element, dataPath) {
  ...
  <block a:for="{{${nodePath}.children || []}}" key="{{item.id}}">
    <template a:if="{{item.props.${constants_1.TEMPLATE_ID} > 0}}" is="{{'TPL_' + item.props.${constants_1.TEMPLATE_ID}}}" data="{{ node: item }}" />
    <template a:else is="{{ 'TPL_' + item.type }}" data="{{ node: item }}" />
  </block>
</block>
<template a:elif="{{${nodePath}.props.${constants_1.TEMPLATE_ID} > 0}}" is="{{'TPL_' + ${nodePath}.props.${constants_1.TEMPLATE_ID}}}" data="{{ node: ${nodePath} }}" />
