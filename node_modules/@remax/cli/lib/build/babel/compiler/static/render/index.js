...
const templates_1 = require("../render/templates");
...
function isRootPath(path) {
    ...
    if (t.isJSXElement(node) &&
        (node.openingElement.name.name === 'block' || !helpers.isHostComponentElement(node, path))) {
        return false;
    }
    // case:
    // 记录根节点
 ...
 * @returns
 */
function markTemplateID(element) {
...
function render(api) {
    ...
    function renderTemplates(path, state) {
        ...
        nodes.forEach(node => {
            ...
            if (templates_1.templateInfoMap.has(templateID)) {
                return;
            }
            templates_1.templateInfoMap.set(templateID, templates_1.createTemplate(api, node, path, ['node']), module, isEntry(path.node));
        });
    }
    return {
        pre(state) {
            templates_1.templateInfoMap.remove(state.opts.filename);
        },
        visitor: {
