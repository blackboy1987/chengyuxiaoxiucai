 ...
 * @returns
 */
function JSXElement(api, node, path, dataPath, createTemplate) {
    ...
    if (helpers.isPlainTextLeaf(element, path)) {
        ...
        childrenTemplate = children
            ...
            .map((child, index) => createTemplate(api, child, path, isExpressionBlock ? dataPath : [...dataPath, 'children', index]))
            .join('\n');
    }
    // 生成模板
    return `<${tag} ${attributes_1.createAttributesTemplate(api, tag, stringPath_1.default(dataPath), attributes)}>${childrenTemplate}</${tag}>\n`;
}
